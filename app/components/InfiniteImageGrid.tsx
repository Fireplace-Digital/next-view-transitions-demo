    ...
    useGSAP(
        () => {
                ...
                scrollTrigger: {
                    ...
                    snap: {
                        x: function(endValue: number) {
                            // Only snap if not dragging
                            if (dragInstanceRef.current?.isDragging) return endValue;
                            const centerElem = document.elementFromPoint(
                                dimensions.winMidX,
                                dimensions.winMidY,
                            );
            ...
            dragInstanceRef.current = Draggable.create(gridRef.current, {
                ...
                onDragStart: () => {
                    // Disable ScrollTrigger when dragging starts
                    smoothScroll.scrollTrigger?.disable();
                },
                onDrag: () => {
                    debouncedCenterCheck(dimensions.winMidX, dimensions.winMidY);
                    updateCenterImage();
                ...
                onDragEnd: () => {
                    // Re-enable ScrollTrigger when dragging ends
                    smoothScroll.scrollTrigger?.enable();
                    snapToNearestImage();
                },
                snap: {
                    x: function(endValue: number) {
            ...
            // Handle wheel events with ScrollTrigger
